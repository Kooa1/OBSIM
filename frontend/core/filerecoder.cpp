...
FileRecoder::FileRecoder() = default;
...
void FileRecoder::feed_frame(const uint8_t *data, int stride, int capture_w, int capture_h) {
...
bool FileRecoder::init_audio_swr(SwrContextPtr &swr, const AVFrame *frame, int out_sample_rate) {
    if (swr) return true;
...
void FileRecoder::encode_video_frame(AVFormatContext *fmt_ctx, AVPacket *pkt,
...
bool FileRecoder::encode_audio_frame(AVFormatContext *fmt_ctx, AVPacket *pkt,
...
bool FileRecoder::init_contexts() {
...
void FileRecoder::process_system_audio() {
...
void FileRecoder::process_mic_audio() {
...
void FileRecoder::mix_audio_streams() {
    size_t mix_length = 0;
...
void FileRecoder::process_video_frame() {
...
void FileRecoder::encode_audio_frames_from_fifo() {
...
void FileRecoder::main_encode_loop() {
    int iter_counter = 0;
...
void FileRecoder::drain_video_frames() {
...
void FileRecoder::drain_audio_residual() {
...
void FileRecoder::flush_encoders_and_write_trailer() {
    AVFormatContext *fmt_ctx = m_fmt_ctx.get();
...
void FileRecoder::encoding_loop() {
    if (!init_contexts()) {
