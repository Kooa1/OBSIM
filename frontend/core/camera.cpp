...
// ========== 设置帧就绪回调 ==========
void Camera::set_frame_ready_callback(FrameReadyCallback callback) {
    std::lock_guard<std::mutex> lock(callback_mutex);
    frame_ready_callback = std::move(callback);
}

// ========== 主线程取帧 ==========
std::optional<AVFramePtr> Camera::try_pop_frame() {
...
void Camera::init_ctx() {
    ...
    if (const int ret = avformat_open_input(&fm_ctx, device_name.c_str(), av_input_format, &options); ret != 0) {
    ...
    // 根据原始像素格式决定是否需要 sws 转换
    if (av_codec_context->pix_fmt != target_pixel_format) {
        cout << "Camera pixel format is not BGRA, need sws_scale\n";
        init_sws_ctx();
        change_fmt = true;
...
// ========== 需要格式转换的接收函数 ==========
void Camera::receive_frame0(AVPacketPtr obj_packet) {
...
// ========== 不需要格式转换的接收函数 ==========
void Camera::receive_frame1(AVPacketPtr obj_packet) {
    auto packet = std::move(obj_packet);
    int ret = avcodec_send_packet(av_codec_context.get(), packet.get());
...
// ========== 通知主线程 ==========
void Camera::notify_frame_ready() {
    std::lock_guard<std::mutex> lock(callback_mutex);
